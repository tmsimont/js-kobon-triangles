...
function KobonGenerator() {
  ...
  function getXBS(haveSoFar, i, x_space, b_range, rec) {
    ...
    // try again if the slope is too close to parallel
    // but don't recurse too many times
    if (!tooclose || rec == 500) {
      return {
          x: x,
          b: b
  ...
  function kobonLines(xbs) {
    ...
    for (var i = 0; i < lines.length; i++) {
      lines[i].intersections.sort(function(a, b) {
        return a.point.x - b.point.x;
      });
      for (var j = 0; j < lines[i].intersections.length - 1; j++) {
        var seg = new segment(lines[i],
                               lines[i].intersections[j],
                               lines[i].intersections[j + 1]);
    ...
    for (var i = 0; i < intersections.length; i++) {
      for (var j = 0; j < intersections[i].segments.length - 1; j++) {
        for (var k = 1 + j; k < intersections[i].segments.length; k++) {
          ...
          // This forces a "perfect" arrangment in which
          // no segment can be part of 2 triangles
          ...
          for (var x = 0; x < intersectA.segments.length; x++) {
            ...
            if (
              intersectA.segments[x].intersect1.point
                .equals(intersectB.point) ||
              intersectA.segments[x].intersect2.point
                .equals(intersectB.point)
            ) {
