...
function KobonSVG(containerId) {
	...
	var SVG = {
		...
		createCanvas : function( width, height ){
			...
			SVG.canvas = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
		...
		createTriangle : function (p1, p2, p3) {
			var aLine = document.createElementNS('http://www.w3.org/2000/svg', 'polygon');
			aLine.setAttribute('points', 
						(p1.x + SVG.canvasWidth - gmax_x) + "," + (p1.y + SVG.canvasHeight - gmax_y) + " "
						+ (p2.x + SVG.canvasWidth - gmax_x) + "," + (p2.y + SVG.canvasHeight - gmax_y) + " "
						+ (p3.x + SVG.canvasWidth - gmax_x) + "," + (p3.y + SVG.canvasHeight - gmax_y)
			);
			aLine.setAttribute('stroke', 'rgb(55,55,55)');
			aLine.setAttribute('stroke-width', '.2');
			return aLine;
		...
		scale : function(width, height) {
			SVG.canvas.setAttribute('width', width);
			SVG.canvas.setAttribute('height', height);
			SVG.canvas.setAttribute("viewBox", "0 0 " + SVG.canvasWidth + " " + SVG.canvasHeight);
		}
	}
